...
const bookingSchema = new mongoose.Schema({
  ...
  ownerResponse: {
    ...
    response: { type: String, enum: ['accepted', 'rejected', 'negotiating'] },
    responseTime: Date,
    ownerNotes: String
  },
  ...
  payment: {
    ...
    paymentDate: Date,
    paymentMethod: { type: String, enum: ['card', 'bank_transfer', 'mobile_banking', 'cash'] },
  ...
  contactInfo: {
    ...
    adminContactedOwner: { type: Boolean, default: false },
    adminContactDate: Date
  },

  // ✅ Lightweight summary that matches your UI payload
  requestSummary: {
    requesteeName: String,   // comes from req.user.name
    totalBill: Number,       // what your form computes/sends
    totalMonths: Number      // what your form computes/sends
  },
...
// Split commission whenever advanceAmount is set
bookingSchema.pre('save', function (next) {
...
// Add timeline when status changes
bookingSchema.pre('save', function (next) {
